...
시스템은 단일 명령으로 FE+BE를 동시에 실행할 수 있어야 한다(MUST). 로컬에서는 SQLite를 사용하며, FastAPI가 정적 파일을 함께 서빙한다.
...
시스템은 Vercel에 FE+BE를 단일 배포로 올릴 수 있어야 한다(MUST). Vercel Storage Neon의 DATABASE_URL이 자동 주입된다.
...
시스템은 앱 시작 시 DB 테이블을 자동으로 생성·확인해야 한다(MUST).

#### Scenario: 최초 실행
- **WHEN** 빈 DB에서 앱 시작 시
- **THEN** users, teams, tasks, messages 4개 테이블 자동 생성
